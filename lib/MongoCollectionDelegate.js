...
export default class MongoCollectionDelegate {
  ...
  _updateMany(queryObject, modifier, coll, {sort = {_id: 1}, upsert = false}) {
  ...
  _updateOne(queryObject, modifier, coll, {upsert = false}) {
    return coll.findOneAndUpdate(queryObject, modifier, { upsert })
      .then((res) => {
        const funcResult = {
          modified: 0,
          original: [],
